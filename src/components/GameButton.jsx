...
const GameButton = ({ number, isRevealed, isBomb, onClick, gameStatus, disabled }) => (
 <button
    id={number}
    onClick={() => onClick(number)}
    className={`md:h-20 md:w-20 h-48 w-48 rounded-2xl md:rounded-xl border-b-4 
  ...
  >
    {isRevealed || gameStatus=='lost' ? <Lottie animationData={isBomb ? bomb : coin} loop={true} className='w-64 md:w-32' /> : <h1 className='text-9xl md:text-5xl'>?</h1>}
  </button>
);
